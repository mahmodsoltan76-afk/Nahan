...
const CURRENT_VERSION = "4.0.0-zavali-bpb-config";
...
// ZAVALI/BPB configuration layer: only client-profile serialization is changed.
// The Nahan WebSocket/telemetry connection core and UUID registry are untouched.
function bpbEncodeQuery(obj) {
  return Object.entries(obj)
    .filter(([,v]) => v !== undefined && v !== null && String(v) !== "")
    .map(([k,v]) => `${encodeURIComponent(k)}=${encodeURIComponent(String(v))}`)
    .join("&");
}
function buildBpbVlessUri({uuid, server, port, security, sni, fp, host, path, name, insecure}) {
  const q = bpbEncodeQuery({
    encryption: "none",
  ...
  return `${getAlpha()}://${uuid}@${server}:${port}?${q}#${encodeURIComponent(name || "ZAVALI VLESS")}`;
}
function buildBpbTrojanUri({password, server, port, security, sni, fp, host, path, name, insecure}) {
  const q = bpbEncodeQuery({
    security: security || "tls",
    sni: sni || server,
    fp: fp || "random",
  ...
  return `${getBeta()}://${password}@${server}:${port}?${q}#${encodeURIComponent(name || "ZAVALI Trojan")}`;
}
            ...
            effectivePorts.forEach((port) => {
                ...
                ips.forEach((ip) => {
                    ...
                    if (effectiveMode === "alpha" || effectiveMode === "both") {
                        ...
                        lines.push(
                            buildBpbVlessUri({uuid: configUuid, server: ip, port, security: sec, sni: hName, fp: sysConfig.agent, host: hName, path: reqPath, name: vName, insecure: allowInsecure}),
                        );
                    }
                    if (effectiveMode === "beta" || effectiveMode === "both") {
                        ...
                        lines.push(
                            buildBpbTrojanUri({password: p.id, server: ip, port, security: sec, sni: hName, fp: sysConfig.agent, host: hName, path: pathStrTr, name: tName, insecure: allowInsecure}),
                        );
                    }
                    if (sysConfig.enableDirectConfigs && pips.length > 0 && selectedProxyIp === pips[0]) {
                        ...
                        if (
                            ...
                            lines.push(
                                buildBpbVlessUri({uuid: configUuid, server: ip, port, security: sec, sni: hName, fp: sysConfig.agent, host: hName, path: reqPath, name: dvName, insecure: allowInsecure}),
                            );
                        }
                        if (
                            effectiveMode === "beta" ||
                            ...
                            lines.push(
                                buildBpbTrojanUri({password: p.id, server: ip, port, security: sec, sni: hName, fp: sysConfig.agent, host: hName, path: pathStrTr2, name: dtName, insecure: allowInsecure}),
                            );
                        }
                    }
                    configIndex++;
