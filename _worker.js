...
const SYSTEM_DEFAULTS = {
    ...
    isPaused: false,
    silentAlerts: false,
    githubRepo: "mahmodsoltan76-afk/zavali",
    nameStrategy: "{USER}{FLAG}{INDEX}{PREFIX}",
    namePrefix: "(M)",
    tgBotLang: "fa",
    users: [],
    subUserAgent: "",
...
function sanitizeConfigForApi(config) {
    const copy = { ...config };
    if (Array.isArray(copy.users)) copy.users = copy.users.map(sanitizeUserForApi);
    return copy;
}
    ...
    async fetch(request, env, ctx) {
        try {
            ...
            const routes = {
                ...
                subscriberLogin: `/${encodeURI(sysConfig.apiRoute)}/login`,
            ...
            const isAuthorizedRoute =
                ...
                reqPath === routes.dash ||
                reqPath === routes.auth ||
                reqPath === routes.subscriberLogin ||
                reqPath === routes.sync ||
                reqPath === routes.tg ||
            ...
            if (!isTelemetryStream) {
                if (reqPath === routes.dash) {
                    const dashboardUrl = env.DASHBOARD_URL || 'https://raw.githubusercontent.com/mahmodsoltan76-afk/Nahan/refs/heads/main/dashboard.html';
                    try {
                ...
                if (reqPath === routes.subscriberLogin) {
                    if (request.method !== "POST")
                        return new Response("405", { status: 405 });
                    return await handleSubscriberLogin(request, url.hostname, ctx, env);
                }
                ...
                if (reqPath === routes.data) {
                    ...
                    if (isRealBrowser && !isCustomUaAllowed) {
                        if (hasMultiUser) {
                            const sessionUser = await getSubscriberSessionUser(request, env);
                            if (!sessionUser || !isValidUser || sessionUser.id !== targetUser.id) {
                                return new Response(
                                    buildSubscriberLoginPage(
                                        targetUser?.name || "",
                                        sessionUser && isValidUser && sessionUser.id !== targetUser.id
                                            ? "این پنل متعلق به کاربر دیگری است. لطفاً دوباره وارد شوید."
                                            : "",
                                    ),
                                    {
                                        status: 401,
                        ...
                        if (isValidUser) {
                            const subscriptionUrl = env.SUBSCRIPTION_URL || 'https://raw.githubusercontent.com/mahmodsoltan76-afk/Nahan/refs/heads/main/subscription.html';
                            try {
    ...
    try {
        ...
        if (method === "POST" && !userId) {
            ...
            const newUser = {
                ...
                ...(body.password ? { passwordHash: await sha256HexText(String(body.password)) } : {}),
                createdAt: Date.now(),
            };
            ...
            return new Response(
                JSON.stringify({
                    success: true,
                    user: sanitizeUserForApi(newUser),
                    subscriptionUrl: subUrl,
                }),
                {
                    status: 201,
        ...
        if (method === "PUT" && userId) {
            ...
            if (body.password !== undefined) {
                if (String(body.password).length > 0) u.passwordHash = await sha256HexText(String(body.password));
...
const subscriberFailedLogins = new Map();
const SUBSCRIBER_SESSION_TTL = 7 * 24 * 60 * 60 * 1000;
...
async function makeSubscriberSession(user, env) {
    const secret = env.SUBSCRIBER_SESSION_SECRET || sysConfig.masterKey || "change-me";
    const payload = `${user.id}|${Date.now() + SUBSCRIBER_SESSION_TTL}`;
    const encoded = base64UrlEncodeText(payload);
    const sig = await hmacSha256Hex(secret, encoded);
    return `${encoded}.${sig}`;
}

async function getSubscriberSessionUser(request, env) {
    const cookieHeader = request.headers.get("Cookie") || "";
    const match = cookieHeader.match(/(?:^|;\s*)nahan_sub_session=([^;]+)/);
    ...
    const payload = base64UrlDecodeText(encoded);
    ...
    return (sysConfig.users || []).find((u) => u.id === userId) || null;
}

function escapeSubscriberHtml(value) {
    return String(value ?? "")
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
...
function buildSubscriberLoginPage(username = "", error = "") {
    const safeUser = escapeSubscriberHtml(username);
    const safeError = escapeSubscriberHtml(error);
    return `<!doctype html>
...
<style>
*{box-sizing:border-box}html,body{margin:0;min-height:100%;font-family:Vazirmatn,Tahoma,sans-serif}body{min-height:100vh;background:radial-gradient(circle at 20% 10%,rgba(244,196,0,.10),transparent 30%),radial-gradient(circle at 80% 90%,rgba(255,216,61,.08),transparent 32%),#11161b;color:#f8fafc;display:flex;align-items:center;justify-content:center;padding:16px;overflow:hidden}.shell{width:100%;min-height:100vh;display:flex;align-items:center;justify-content:center;position:relative}.glow{position:absolute;width:260px;height:260px;border-radius:50%;background:#f4c400;filter:blur(100px);opacity:.10;pointer-events:none}.g1{top:-100px;right:-90px}.g2{bottom:-120px;left:-100px}.card{width:min(430px,100%);position:relative;border:1px solid rgba(255,216,61,.18);border-radius:28px;background:rgba(24,30,36,.92);box-shadow:0 24px 80px rgba(0,0,0,.38);padding:34px;backdrop-filter:blur(16px)}.brand{width:76px;height:76px;margin:0 auto 18px;border-radius:24px;display:flex;align-items:center;justify-content:center;background:linear-gradient(145deg,#ffe66d,#f4c400 55%,#d9aa00);box-shadow:0 12px 30px rgba(244,196,0,.28);font-size:30px;font-weight:900;color:#17202a}.title{margin:0;text-align:center;font-size:24px;font-weight:900;letter-spacing:-.02em}.subtitle{text-align:center;margin:9px 0 24px;font-size:13px;color:#aeb7c1;line-height:1.9}.label{display:block;margin:15px 0 7px;font-size:13px;font-weight:700;color:#d7dee5}.input{width:100%;height:50px;border:1px solid #37414a;border-radius:15px;background:#11161b;color:#f8fafc;padding:0 15px;font:600 14px Vazirmatn,Tahoma,sans-serif;outline:none;transition:.2s}.input:focus{border-color:#f0c400;box-shadow:0 0 0 4px rgba(244,196,0,.13)}.button{width:100%;height:52px;margin-top:22px;border:0;border-radius:15px;background:linear-gradient(135deg,#f7d21b,#e8b900);color:#17202a;font:900 14px Vazirmatn,Tahoma,sans-serif;cursor:pointer;box-shadow:0 10px 24px rgba(244,196,0,.24);transition:.2s}.button:hover{transform:translateY(-1px);background:linear-gradient(135deg,#ffe05a,#d9aa00)}.button:disabled{opacity:.65;cursor:wait;transform:none}.error{display:flex;align-items:center;gap:9px;margin-bottom:14px;padding:12px 13px;border-radius:13px;background:rgba(244,63,94,.10);border:1px solid rgba(244,63,94,.22);color:#fda4af;font-size:12px;line-height:1.8}.footer{text-align:center;margin-top:18px;font-size:11px;color:#7f8a95}@media(max-width:480px){.card{padding:27px 20px;border-radius:23px}.title{font-size:21px}}
</style></head>
<body><main class="shell"><div class="glow g1"></div><div class="glow g2"></div><form class="card" id="loginForm" autocomplete="on">
<div class="brand">Z</div><h1 class="title">ورود به پنل کاربری</h1><p class="subtitle">برای مشاهده اطلاعات اشتراک و مدیریت سرویس، وارد حساب کاربری خود شوید.</p>
${safeError ? `<div class="error"><span>⚠️</span><span>${safeError}</span></div>` : ""}
<label class="label" for="username">نام کاربری</label><input class="input" id="username" name="username" value="${safeUser}" autocomplete="username" autocapitalize="none" spellcheck="false" placeholder="نام کاربری خود را وارد کنید" required>
<label class="label" for="password">رمز عبور</label><input class="input" id="password" name="password" type="password" autocomplete="current-password" placeholder="رمز عبور خود را وارد کنید" required>
<button class="button" type="submit"><span id="btnText">ورود به پنل</span></button><div class="footer">دسترسی امن به پنل اختصاصی شما</div>
</form></main><script>
const form=document.getElementById("loginForm");
form.addEventListener("submit",async function(e){e.preventDefault();const username=document.getElementById("username").value.trim();const password=document.getElementById("password").value;const returnTo=location.pathname+location.search;const btn=this.querySelector("button");const txt=document.getElementById("btnText");btn.disabled=true;txt.textContent="در حال ورود…";try{const r=await fetch(location.pathname.replace(/\\/$/,"")+"/login",{method:"POST",headers:{"Content-Type":"application/json"},credentials:"same-origin",body:JSON.stringify({username,password,returnTo})});const data=await r.json().catch(()=>({}));if(r.ok&&data.success){location.href=data.redirect||returnTo;return}alert(data.error||"نام کاربری یا رمز عبور اشتباه است.")}catch(e){alert("خطا در اتصال. لطفاً دوباره تلاش کنید.")}finally{btn.disabled=false;txt.textContent="ورود به پنل"}});
</script></body></html>`;
}

async function handleSubscriberLogin(request, hostName, ctx, env) {
    try {
        ...
        const now = Date.now();
        const state = subscriberFailedLogins.get(rateKey) || { count: 0, until: 0 };
        ...
        const user = (sysConfig.users || []).find(
            (u) => String(u.name || "").toLowerCase() === username.toLowerCase(),
        );

        let valid = false;
        if (user && !user.isPaused && !(user.expiryMs && Date.now() > user.expiryMs)) {
            if (user.passwordHash) {
                valid = (await sha256HexText(password)) === user.passwordHash;
            } else if (user.password) {
                // Migrate legacy plain password to a hash after a successful login.
                valid = password === String(user.password);
                if (valid) {
                    user.passwordHash = await sha256HexText(password);
        ...
        if (!valid) {
            ...
            subscriberFailedLogins.set(rateKey, state);
            ctx?.waitUntil(
                logActivity(env, "Subscriber Auth Failed", `Failed subscriber login for "${username}" from ${ip}`).catch(() => {}),
            );
        ...
        subscriberFailedLogins.delete(rateKey);
        const token = await makeSubscriberSession(user, env);

        const defaultReturn = `/${encodeURI(sysConfig.apiRoute)}?sub=${encodeURIComponent(user.name)}`;
        const safeReturn = requestedReturnTo.startsWith(`/${sysConfig.apiRoute}`)
            ? requestedReturnTo
            : defaultReturn;

        ctx?.waitUntil(
            logActivity(env, "Subscriber Auth Success", `Successful subscriber login for "${user.name}" from ${ip}`).catch(() => {}),
        );

        return new Response(JSON.stringify({ success: true, redirect: safeReturn }), {
            status: 200,
            headers: {
                "Content-Type": "application/json",
                "Cache-Control": "no-store",
                "Set-Cookie": `nahan_sub_session=${encodeURIComponent(token)}; Path=/${encodeURI(sysConfig.apiRoute)}; Max-Age=${Math.floor(SUBSCRIBER_SESSION_TTL / 1000)}; HttpOnly; Secure; SameSite=Lax`,
            },
        });
    } catch (e) {
            ...
            return new Response(
                JSON.stringify({
                    ...
                    config: isPanelApiKey(loginKey)
                        ? {
                              ...sanitizeConfigForApi(sysConfig),
                              masterKey: "[PROTECTED]",
    ...
    try {
        ...
        if (data.config) {
            ...
            const previousUsersById = new Map(
                (sysConfig.users || []).map(u => [String(u.id), u]),
            );
            nextConfig = { ...sysConfig, ...data.config };
            if (Array.isArray(nextConfig.users)) {
                const normalizedUsers = [];
                for (const incomingUser of nextConfig.users) {
                    const u = { ...incomingUser };
                    const previousUser = previousUsersById.get(String(u.id));
                    if (typeof u.password === "string" && u.password.length > 0) {
                        u.passwordHash = await sha256HexText(u.password);
                    } else if (previousUser?.passwordHash && !u.passwordHash) {
                        u.passwordHash = previousUser.passwordHash;
                    } else if (previousUser?.password && !u.password) {
                        u.passwordHash = await sha256HexText(String(previousUser.password));
                    }
                    delete u.password;
                    normalizedUsers.push(u);
                }
                nextConfig.users = normalizedUsers;
            }
            if (
...
function parseShadowsocksUri(uri) {
    if (!uri || typeof uri !== "string" || !uri.trim().toLowerCase().startsWith("ss://")) return null;
    try {
        let rest = uri.trim().slice(5);
        let fragment = "";
        const hash = rest.indexOf("#");
        if (hash >= 0) { fragment = decodeURIComponent(rest.slice(hash + 1)); rest = rest.slice(0, hash); }
        let plugin = "", pluginOpts = "";
        const q = rest.indexOf("?");
        if (q >= 0) {
            const qs = new URLSearchParams(rest.slice(q + 1));
        ...
        if (at >= 0) {
            const decoded = base64UrlDecode(rest.slice(0, at));
            const colon = decoded.indexOf(":");
            if (colon < 0) return null;
            method = decoded.slice(0, colon); password = decoded.slice(colon + 1);
            const hp = rest.slice(at + 1);
            if (hp.startsWith("[")) { const e = hp.indexOf("]"); server = hp.slice(1,e); port = parseInt(hp.slice(e+2),10); }
            else { const c = hp.lastIndexOf(":"); server = hp.slice(0,c); port = parseInt(hp.slice(c+1),10); }
        } else {
            const decoded = base64UrlDecode(rest);
            const c = decoded.indexOf(":");
            if (c < 0) return null;
            const auth = decoded.slice(0,c); const hp = decoded.slice(c+1);
            const cc = auth.indexOf(":");
            if (cc < 0) return null;
            method = auth.slice(0,cc); password = auth.slice(cc+1);
            if (hp.startsWith("[")) { const e = hp.indexOf("]"); server = hp.slice(1,e); port = parseInt(hp.slice(e+2),10); }
            else { const x = hp.lastIndexOf(":"); server = hp.slice(0,x); port = parseInt(hp.slice(x+1),10); }
        }
        if (!server || !port || !method || password === "") return null;
        return { name: fragment || "Shadowsocks", server, port, method, password, plugin, pluginOpts, raw: uri };
    } catch (e) { return null; }
}
function buildShadowsocksUri(server, port, method, password, plugin, pluginOpts, name) {
    if (!server || !port || !method || !password) return "";
    let uri = `ss://${base64UrlEncode(`${method}:${password}`)}@${server.includes(":") && !server.startsWith("[") ? `[${server}]` : server}:${parseInt(port,10) || 443}/`;
    if (plugin) uri += `?plugin=${encodeURIComponent(pluginOpts ? `${plugin};${pluginOpts}` : plugin)}`;
    if (name) uri += `#${encodeURIComponent(name)}`;
    return uri;
}
function getExternalRawShadowsocks(profile) {
    return getExternalRawConfigs(profile).map(raw => ({ raw, parsed: parseShadowsocksUri(raw) })).filter(x => x.parsed);
}
function profileShadowsocks(profile) {
    if (!profile || !profile.ssEnabled || !profile.ssServer || !profile.ssPassword) return null;
    return { name: profile.name || "Shadowsocks", server: profile.ssServer, port: parseInt(profile.ssPort,10) || 443, method: profile.ssMethod || "2022-blake3-aes-256-gcm", password: profile.ssPassword, plugin: profile.ssPlugin || "", pluginOpts: profile.ssPluginOpts || "" };
}
function shadowsocksToClashProxy(parsed) {
    if (!parsed) return null;
    const ob = { name: parsed.name || "Shadowsocks", type: "ss", server: parsed.server, port: parsed.port, cipher: parsed.method, password: parsed.password, udp: true };
    if (parsed.plugin) { ob.plugin = parsed.plugin.split(";")[0]; if (parsed.pluginOpts) ob["plugin-opts"] = Object.fromEntries(parsed.pluginOpts.split(";").filter(Boolean).map(x => { const i=x.indexOf("="); return i<0 ? [x,true] : [x.slice(0,i),x.slice(i+1)]; })); }
    return ob;
}
function shadowsocksToSingboxOb(parsed) {
    if (!parsed) return null;
    const ob = { type: "shadowsocks", tag: "🔗 " + (parsed.name || "Shadowsocks"), server: parsed.server, server_port: parsed.port, method: parsed.method, password: parsed.password, udp_over_tcp: { enabled: false } };
    if (parsed.plugin) { ob.plugin = parsed.plugin.split(";")[0]; if (parsed.pluginOpts) ob.plugin_opts = parsed.pluginOpts; }
    return ob;
}
function shadowsocksToV2RayOb(parsed) {
    if (!parsed) return null;
    return { tag: "🔗 " + (parsed.name || "Shadowsocks"), protocol: "shadowsocks", settings: { servers: [{ address: parsed.server, port: parsed.port, method: parsed.method, password: parsed.password, ota: false }] } };
}

function getExternalRawConfigs(profile) {
    if (!profile || typeof profile.externalRawConfigs !== "string") return [];
    return profile.externalRawConfigs
        .split(/[\r\n]+/)
        .map((s) => s.trim())
        .filter(Boolean);
}

function getExternalRawVless(profile) {
    return getExternalRawConfigs(profile)
        .map((raw) => ({ raw, parsed: parseVlessUri(raw) }))
        .filter((x) => x.parsed);
}

function externalRawClashYaml(parsed) {

    const name = String(parsed.name || "External Raw").replace(/"/g, '""');
    const path = String(parsed.path || "/").replace(/"/g, '\\"');
    const host = String(parsed.host || parsed.sni || parsed.server).replace(/"/g, '\\"');
        ...
        sysConfig.users.forEach((u) => {
            ...
            if (!skip) {
                list.push({
                    ...
                    externalRawConfigs: u.externalRawConfigs || "",
                    ssEnabled: u.ssEnabled || false,
                    ssServer: u.ssServer || "",
                    ssPort: u.ssPort || 443,
                    ssMethod: u.ssMethod || "2022-blake3-aes-256-gcm",
                    ssPassword: u.ssPassword || "",
                    ssPlugin: u.ssPlugin || "",
                    ssPluginOpts: u.ssPluginOpts || "",
                });
...
async function buildUriProfile(
    ...
    profiles.forEach((p) => {
        const ss = profileShadowsocks(p);
        if (ss) lines.push(buildShadowsocksUri(ss.server, ss.port, ss.method, ss.password, ss.plugin, ss.pluginOpts, ss.name));
    ...
    // ─── External Raw Configs: append per-user raw configs ───
    profiles.forEach((p) => {
        getExternalRawConfigs(p).forEach((raw) => lines.push(raw));
    });
...
async function buildYamlProfile(hostName, targetSub = null, allowInsecure = false, env = null) {
    ...
    // ─── External Raw Configs: append per-user VLESS configs ───
    profiles.forEach((p) => {
        getExternalRawVless(p).forEach(({ parsed }) => proxies.push(externalRawClashYaml(parsed)));
    });
...
async function buildClashJsonProfile(
    ...
    // ─── External Raw Configs: append per-user VLESS configs ───
    profiles.forEach((p) => {
        getExternalRawVless(p).forEach(({ parsed }) => {
            const ob = upstreamToClashProxy(parsed);
            if (ob) proxiesArr.push(ob);
        });
    });
...
async function buildVJsonProfile(hostName, targetSub = null, allowInsecure = false, env = null) {
    ...
    profiles.forEach((p) => {
        const ss = profileShadowsocks(p);
        if (ss) {
            const proxy = shadowsocksToClashProxy(ss);
            if (proxy) { proxy.name = getUniqueName(proxy.name); proxiesArr.push(proxy); dynamicTags.push(proxy.name); proxyGeoInfo.set(proxy.name, getGeoInfo(ss.server)); }
        }
        getExternalRawShadowsocks(p).forEach(({parsed}) => {
            const proxy = shadowsocksToClashProxy(parsed);
            if (proxy) { proxy.name = getUniqueName(proxy.name); proxiesArr.push(proxy); dynamicTags.push(proxy.name); proxyGeoInfo.set(proxy.name, getGeoInfo(parsed.server)); }
        });
    });
    ...
    profiles.forEach((p) => {
        const ss = profileShadowsocks(p);
        if (ss) { const ob = shadowsocksToV2RayOb(ss); if (ob) outboundsArr.push(ob); }
        getExternalRawShadowsocks(p).forEach(({parsed}) => { const ob = shadowsocksToV2RayOb(parsed); if (ob) outboundsArr.push(ob); });
    });
    ...
    // ─── External Raw Configs: append per-user VLESS + Shadowsocks configs ───
    profiles.forEach((p) => {
        getExternalRawVless(p).forEach(({ parsed }) => { const ob = upstreamToV2RayOb(parsed); if (ob) outboundsArr.push(ob); });
        getExternalRawShadowsocks(p).forEach(({ parsed }) => { const ob = shadowsocksToV2RayOb(parsed); if (ob) outboundsArr.push(ob); });
    });
...
async function buildSingBoxJsonProfile(hostName, targetSub = null, allowInsecure = false, env = null) {
    ...
    profiles.forEach((p) => {
        const ss = profileShadowsocks(p);
        if (ss) { const ob = shadowsocksToSingboxOb(ss); if (ob) outboundsArr.push(ob); }
        getExternalRawShadowsocks(p).forEach(({parsed}) => { const ob = shadowsocksToSingboxOb(parsed); if (ob) outboundsArr.push(ob); });
    });
    ...
    // ─── External Raw Configs: append per-user VLESS + Shadowsocks configs ───
    profiles.forEach((p) => {
        getExternalRawVless(p).forEach(({ parsed }) => { const ob = upstreamToSingboxOb(parsed); if (ob) outboundsArr.push(ob); });
        getExternalRawShadowsocks(p).forEach(({ parsed }) => { const ob = shadowsocksToSingboxOb(parsed); if (ob) outboundsArr.push(ob); });
    });
