...
// Apply the same naming strategy/prefix used by generated configs to external raw configs.
function getExternalRawConfigName(parsed, profile, configIndex = 0) {
    const baseName = parsed && parsed.name ? parsed.name : "External";
    const server = parsed?.server || "";
    const port = parsed?.port || "";
    const host = parsed?.sni || parsed?.host || server;
...
function renameExternalRawUri(raw, name) {
    const base = String(raw || "").split("#")[0];
    return `${base}#${encodeURIComponent(name)}`;
}

function externalRawClashYaml(parsed, nameOverride = null) {
    const name = String(nameOverride || parsed.name || "External Raw").replace(/"/g, '""');
...
async function buildUriProfile(
    ...
    let externalRawIndex = 0;
    profiles.forEach((p) => {
        getExternalRawConfigs(p).forEach((raw) => {
            const parsed = parseVlessUri(raw);
            if (parsed) {
                const rawName = getExternalRawConfigName(parsed, p, externalRawIndex++);
                lines.push(renameExternalRawUri(raw, rawName));
            } else {
                // Preserve unsupported raw schemes as-is; VLESS receives the normal naming strategy.
                lines.push(raw);
            }
        });
    });
...
async function buildYamlProfile(hostName, targetSub = null, allowInsecure = false, env = null) {
    ...
    profiles.forEach((p) => {
        getExternalRawVless(p).forEach(({ parsed }) => {
            const rawName = getExternalRawConfigName(parsed, p, externalRawIndex++);
            proxies.push(externalRawClashYaml(parsed, rawName));
        });
    });
...
async function buildClashJsonProfile(
    ...
    profiles.forEach((p) => {
        getExternalRawVless(p).forEach(({ parsed }) => {
            const rawName = getExternalRawConfigName(parsed, p, externalRawIndex++);
            const ob = upstreamToClashProxy({ ...parsed, name: rawName });
...
async function buildVJsonProfile(hostName, targetSub = null, allowInsecure = false, env = null) {
    ...
    profiles.forEach((p) => {
        getExternalRawVless(p).forEach(({ parsed }) => {
            const rawName = getExternalRawConfigName(parsed, p, externalRawIndex++);
            const ob = upstreamToV2RayOb({ ...parsed, name: rawName });
            if (ob) outboundsArr.push(ob);
        });
    });
...
async function buildSingBoxJsonProfile(hostName, targetSub = null, allowInsecure = false, env = null) {
    ...
    profiles.forEach((p) => {
        getExternalRawVless(p).forEach(({ parsed }) => {
            const rawName = getExternalRawConfigName(parsed, p, externalRawIndex++);
            const ob = upstreamToSingboxOb({ ...parsed, name: rawName });
            if (ob) outboundsArr.push(ob);
        });
    });
